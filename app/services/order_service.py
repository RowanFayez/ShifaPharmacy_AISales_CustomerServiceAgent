...
def quote_order(items: list[dict]) -> dict:
    """Return current checkout totals without creating an order or changing stock."""
    if not items:
        raise ServiceError("INVALID_ORDER", "At least one item is required.")
    requested: dict[int, int] = {}
    for item in items:
        product_id, quantity = int(item["product_id"]), int(item["quantity"])
        if quantity < 1:
            raise ServiceError("INVALID_QUANTITY", "Quantity must be at least one.")
        requested[product_id] = requested.get(product_id, 0) + quantity
    products = {product.id: product for product in db.session.scalars(select(Product).where(Product.id.in_(requested))).all()}
    if len(products) != len(requested):
        raise ServiceError("PRODUCT_NOT_FOUND", "One or more products were not found.")
    for product_id, quantity in requested.items():
        product = products[product_id]
        if not product.active or product.requires_prescription or product.stock_quantity < quantity:
            raise ServiceError("PRODUCT_UNAVAILABLE", f"{product.name} cannot be ordered.")
    subtotal = sum((products[product_id].price * quantity for product_id, quantity in requested.items()), Decimal("0.00"))
    delivery_fee = Decimal("0.00") if subtotal >= Decimal("500.00") else Decimal("35.00")
    return {"subtotal": str(subtotal), "delivery_fee": str(delivery_fee), "total": str(subtotal + delivery_fee)}
